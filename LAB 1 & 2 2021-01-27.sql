...
with cte_actor1 as (
  select fi.film_id, fi.actor_id as actor1, ai.first_name, ai.last_name
  from sakila.film_actor as fi
  join actor as ai
  on fi.actor_id=ai.actor_id
)
select 	cte.film_id, cte.actor1, cte.first_name, cte.last_name, 
		f.actor_id as actor2, ao.first_name, ao.last_name from cte_actor1 as cte
join film_actor as f
on cte.film_id=f.film_id
and actor1<f.actor_id
join actor as ao
on f.actor_id=ao.actor_id
order by cte.film_id, cte.actor1
;

#For each film, list actor that has acted in more films;
select f1.film_id, f1.title, f2.actor_id from film as f1
join film_actor as f2
on f1.film_id=f2.film_id
where f2.actor_id in (select actor_id from
	(
	select actor_id, count(film_id) as n_films from film_actor
	group by actor_id
